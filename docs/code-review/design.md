...
Claude는 변경 diff, 결정적 검사 결과, 변경 파일 전체 내용, 그리고 재사용 후보 파일을 받는다.

- 변경 파일은 전부 첨부한다. 파일 개수 제한과 전체 용량 제한은 없다(S15P11A105-215).
- 파일당 256KB 상한만 둔다. 넘는 파일은 건너뛰지 않고 앞부분만 첨부하며 잘렸다는 사실을 문맥과 리포트에 남긴다. 생성 파일 하나가 컨텍스트와 메모리를 다 쓰는 것을 막되, 손으로 쓴 코드(최대 65KB)는 잘리지 않는 값이다.
- 상한을 넘는 파일은 전부 읽어 잘라내지 않고 앞부분만 읽으므로, 13MB 모델 가중치가 변경돼도 버퍼가 상한을 넘지 않는다.
- 삭제된 파일, 텍스트가 아닌 파일(NUL 바이트), 저장소 루트 밖 경로만 건너뛰고 그 이유를 문맥과 리포트에 남긴다.
- 경로가 저장소 안이어도 심볼릭 링크의 실제 경로가 밖이면 읽지 않는다. `fs.realpathSync`로 링크를 따라간 뒤 확인하므로, 저장소 밖 파일 내용이 Claude 프롬프트와 GitLab 댓글로 새지 않는다.
- 재사용 후보는 `fe/src/shared/ui`에서 변경된 JSX와 이름이 맞는 파일을 선택한다.
- 후보 탐색 시 `.git`, `node_modules`, 빌드 산출물은 제외한다.

diff 본문에는 35만 자 상한이 따로 있다. diff는 잘려도 변경 파일 내용은 그대로 들어가므로 검토 근거가 사라지지 않는다.
...
따라서 Claude는 변경과 무관한 저장소 전체를 뒤지지 않는다. 후보에 실제 정의가 있을 때만 재사용 또는 중복 권고를 작성하고, 변경 파일 전체를 읽어도 이번 변경과 무관한 기존 결함은 지적하지 않도록 프롬프트와 `.claude` 역할 파일에 명시한다.
