    ...
    public SecurityFilterChain securityFilterChain(
        ...
        http.csrf(csrf -> csrf.disable())
                ...
                .authorizeHttpRequests(authorize -> authorize
                        ...
                        .permitAll()
                        // 녹화 재생·썸네일. <video> 와 <img> 는 Authorization 헤더를 싣지 못해 자격이 주소에 들어 있고,
                        // 유스케이스가 세션·만료에 묶인 서명으로 검증한다(불일치·만료 401). 주소 발급 API
                        // (/media-url, 목록의 thumbnailUrl)는 여기 열지 않는다 — 그쪽은 로그인과 세션 참여를 확인해야 한다.
                        .requestMatchers(HttpMethod.GET, "/api/v1/sessions/*/media", "/api/v1/sessions/*/thumbnail")
                        .permitAll()
