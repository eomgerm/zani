...
public class SessionEndController {
    ...
    @Operation(summary = "수업 종료 (강사 전용)", description = """
                    강의실 상단의 **수업 종료** 버튼이 호출하는 API입니다. 수업을 연 강사만 호출할 수 있고, 성공하면 세션 상태가 `LIVE` → `ENDED` 로 바뀝니다.
                    ...
                    **두 번 눌러도 안전합니다**
                    이미 종료된 수업에 다시 요청해도 오류가 아니라 `200` 과 함께 `ended: false` 를 돌려줍니다. 버튼 연타·재시도에 안전하도록 멱등하게 동작합니다.
