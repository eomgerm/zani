...
/**
 * 답안 일괄 제출과 채점. 인가·로드는 조회 쪽과 {@link StudentQuizAccess} 로 공유하고, 채점은 선택한 보기의 정답 여부 비교뿐이라 별도 도메인 계층 없이 여기서 끝낸다(티켓의 relaxed
 * 결정).
 *
...
public class SubmitQuizAnswersService implements SubmitQuizAnswersUseCase {
    ...
    public SubmitQuizAnswersResult submit(SubmitQuizAnswersCommand command) {
        StudentQuizSnapshot snapshot = studentQuizAccess.loadOwnQuiz(command.sessionId(), command.memberId());
