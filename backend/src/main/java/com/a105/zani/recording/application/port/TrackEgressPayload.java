 ...
 *
 * <p>V12 이전에 쌓인 pending 행에는 이 필드가 없어 null 로 역직렬화된다. 그런 payload 는 {@code RecordingOrchestrator} 가 <b>LiveKit 을 호출하기
 * 전에</b> 거부한다 — Egress 를 먼저 띄우고 나서 거부하면 실행은 시작됐는데 {@code recordings} 행이 없어 추적할 수 없게 된다. 거부된 행은 outbox 재시도로 남고, 재시도 상한을
 * 넘기면 그 트랙만 녹화되지 않는다.
 */
