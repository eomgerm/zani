 ...
 *     <p>저장된 32개 트랙에서 실제 발화 최댓값은 {@code 0.921} 이고 {@code 0.98} 이상 9건은 전부 환각이었다. 별도 GMS 응답에서 관측된 {@code 0.964} 실제 발화도
 *     임곗값 미만이라 필터 후보가 되지 않는다. 연속성 rescue 는 임곗값 이상이어도 실제 발화와 이어지는 조각을 위한 별도 안전장치다. 따라서 {@code 0.98} 은 반복 규칙이 놓친 완전 무음
 *     구간의 극단값만 정리하는 보조 임곗값이다
