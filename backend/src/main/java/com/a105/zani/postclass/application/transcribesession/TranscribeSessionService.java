...
public class TranscribeSessionService implements TranscribeSessionUseCase {
     ...
     *
     * <p>실측에서 두 값이 소수점까지 일치했다(326.19s / 276.40s). 어긋난다면 우리가 올린 바이트가 우리가 생각한 구간이 아니라는 뜻이고, 그대로 두면 시간축이 밀린 전사가 저장된다. 다만
     * 정수 초 단위로 응답하는 파일이 있어(실측) 반올림 폭은 허용한다 — 조립 쪽 clamp 와 같은 이유, 같은 값이다.
     */
