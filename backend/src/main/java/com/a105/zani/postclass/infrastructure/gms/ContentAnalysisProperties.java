 ...
 * §4.1).
 *
 * @param maxCompletionTokens 요약과 구간 목록의 출력 상한. 스키마가 허용하는 최악(구간 40개 × (제목 200자 + 요약 400자) + 전체 요약 1,500자)을 덮는 값이어야 한다 —
 *     잘리면 {@code finish_reason=length} 로 JSON 이 깨지고 그것은 재시도할 수 없는 실패다
 */
