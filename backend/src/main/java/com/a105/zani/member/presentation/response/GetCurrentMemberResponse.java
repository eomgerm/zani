...
public record GetCurrentMemberResponse(String email, String displayName, String profileImageUrl) {

    public static GetCurrentMemberResponse from(GetCurrentMemberResult result) {
        return new GetCurrentMemberResponse(result.email(), result.displayName(), result.profileImageUrl());
    }
}
