 ...
 *
 * <p><b>{@code session_participants}·{@code coaching_histories} 는 report 소유가 아니다.</b> 의도한 읽기 전용 프로젝션이며 네이티브 SQL 로 세기만
 * 하고 쓰지 않는다 — {@code InstructorAnalysisContextQueryAdapter}(250) 와 같은 방식이다. 세는 값 둘을 위해 세션 도메인에 유스케이스를 새로 뚫으면, 읽기 하나 늘
 * 때마다 소유 도메인에 조회 API 가 하나씩 생긴다.
 *
...
public class InstructorReportQueryAdapter implements InstructorReportQueryPort {
    ...
    private static final String COUNT_ALERTS = """
            SELECT COUNT(*) FROM coaching_histories WHERE session_id = ?
            """;
