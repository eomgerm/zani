...
- **~~어느 seed를 배포할지는 이 일감이 정하지 않습니다.~~** S15P11A105-301에서 **seed 49**로
  정했습니다. Validation 다섯 축(accuracy·macro-F1·QWK·저참여 recall·FPR)을 표준화한 거리에서
  10-seed 평균에 가장 가까운 medoid이므로, README가 보고한 집계 수치가 곧 이 seed의 거동입니다.
  `alpha = 0`이라 혼합 기구를 실제로 쓰고, 90분당 오탐 0.064회로 예산 0.5회에 여유가 큽니다.
  Test는 보지 않았습니다. recall 최대인 seed 44는 `alpha = 1`(기준선 그대로)이라 E0-M을
  배포하는 것이 아니고, 이득 최대인 seed 42는 Validation 전체 이득의 3분의 1을 혼자 만든
  이상치입니다. 5-seed 런타임 앙상블은 여전히 범위 밖입니다.
