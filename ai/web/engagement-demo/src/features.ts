import type { FrameLandmarkerValues, LandmarkPoint } from "./contracts";
...
export function extractFrameFeatures(input: FrameLandmarkerValues): Float32Array {
  ...
  const interocular = Math.max(Math.hypot(rightOuter.x - leftOuter.x, rightOuter.y - leftOuter.y), EPSILON);
