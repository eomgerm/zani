...
/** 기본은 확인이 통과하는 서버. 거절 흐름을 보는 테스트만 따로 넘긴다. */
const renderHome = (checkJoinable = vi.fn().mockResolvedValue(joinable)) => {
  render(
    <HomeScreen
      requestSessionList={vi.fn().mockResolvedValue([])}
      checkJoinable={checkJoinable}
    />,
  );
  return checkJoinable;
};
...
describe("HomeScreen 강의실 참여하기", () => {
  ...
  it("코드만 넣어도 되고 소문자·하이픈을 허용한다", async () => {
    const checkJoinable = renderHome();
    ...
    await waitFor(() => expect(pushMock).toHaveBeenCalledWith("/prejoin/GPH7GQ5Q"));
    // 서버에는 정규화한 코드가 간다.
    expect(checkJoinable).toHaveBeenCalledWith("GPH7GQ5Q", "access-token");
  });
  ...
  it("형식이 어긋나면 화면을 옮기지 않고 여기서 알려준다", () => {
    ...
    expect(checkJoinable).not.toHaveBeenCalled();
  });
