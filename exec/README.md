...
> 기준 브랜치: `dev`
>
> 기준 커밋: `c4c1223a9574455e45a4c43af6bb719eccf4f45b`
>
> 기준 일자: 2026-08-07
>
...
| 파일 | 목적 |
| --- | --- |
| `exec/README.md` | 빌드·배포·환경 설정·복구 절차 |
| `exec/environment-variables.md` | 전체 환경변수·운영값·Docker secret 주입 명세 |
| `exec/external-services.md` | Google OAuth, SSAFY GMS, SMTP 등 외부 서비스 설정 |
| `exec/db/zani-schema.sql` | Flyway V1~V20을 적용한 최신 DB 구조 덤프 |
| `exec/demo-scenario.md` | 발표·시연 순서와 정상 결과 |
...
| Backend | Java 21, Spring Boot 4.1.0, Gradle Wrapper 9.5.1, Spring Security, JPA, Flyway, WebSocket |
...
| IDE | 저장소에서 버전을 고정하지 않음. 팀 실제 사용 버전은 제출 전 확인(IntelliJ IDEA/VS Code) |

IDE는 빌드 계약에 포함하지 않는다. IntelliJ IDEA 또는 VS Code를 사용할 수 있지만, 제출·배포 재현의 정본은 아래 CLI 명령과 Java/Node/Python 버전이다.
...
비밀번호는 포팅 매뉴얼이나 DB 덤프에 포함하지 않는다. 최신 구조는 [`db/zani-schema.sql`](db/zani-schema.sql)을 사용한다.
...
전체 사용자 시나리오는 [`demo-scenario.md`](demo-scenario.md)를 따른다.
